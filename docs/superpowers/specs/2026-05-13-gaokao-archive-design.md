...
| 传统文理 | 文科、理科（2选1） | 系统自动填充固定科目 |

**传统文理模式特殊处理**：
- 用户选择 **文科** → 系统自动填充：subject_type=政治, second=历史, third=地理
- 用户选择 **理科** → 系统自动填充：subject_type=物理, second=化学, third=生物

前端只需展示"文科/理科"选项，后端保存时自动映射为对应的三个科目。
