...
public interface MilitaryPositionMapper extends BaseMapper<MilitaryPosition> {
     ...
     */
    @Delete("DELETE FROM t_military_position WHERE id = #{id}")
    int physicalDeleteById(@Param("id") Long id);

    @Delete("<script>DELETE FROM t_military_position WHERE id IN <foreach collection='ids' item='id' open='(' separator=',' close=')'>#{id}</foreach></script>")
