...
public interface CommunityPositionMapper extends BaseMapper<CommunityPosition> {
     ...
     */
    @Delete("DELETE FROM t_community_position WHERE id = #{id}")
    int physicalDeleteById(@Param("id") Long id);

    @Delete("<script>DELETE FROM t_community_position WHERE id IN <foreach collection='ids' item='id' open='(' separator=',' close=')'>#{id}</foreach></script>")
