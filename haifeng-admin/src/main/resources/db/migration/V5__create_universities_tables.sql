...
-- 院校详情表注释（UNIQUE约束已自动创建索引，无需额外索引）
COMMENT ON TABLE t_universities_detail IS '院校详情表（与院校主表1:1）';
COMMENT ON COLUMN t_universities_detail.university_id IS '关联院校ID';
COMMENT ON COLUMN t_universities_detail.address IS '学校地址';
COMMENT ON COLUMN t_universities_detail.admission_phone IS '招生电话';
COMMENT ON COLUMN t_universities_detail.website IS '官方网站';
COMMENT ON COLUMN t_universities_detail.history_group_score IS '本科批历史组分数线';
COMMENT ON COLUMN t_universities_detail.science_group_score IS '本科批物理组分数线';
COMMENT ON COLUMN t_universities_detail.carousel_images IS '轮播图片URL数组';
COMMENT ON COLUMN t_universities_detail.introduction IS '院校详细介绍';
COMMENT ON COLUMN t_universities_detail.rankings IS '排名信息JSONB（软科/校友会/武书连/QS/USNEWS）';
COMMENT ON COLUMN t_universities_detail.abroad_rate IS '出国比例';
COMMENT ON COLUMN t_universities_detail.gender_ratio IS '男女比例';
...
-- 院校适应指南表注释
COMMENT ON TABLE t_university_guides IS '院校适应指南表（与院校主表1:1）';
COMMENT ON COLUMN t_university_guides.university_id IS '关联院校ID';
COMMENT ON COLUMN t_university_guides.custom_tags IS '自定义标签数组';
COMMENT ON COLUMN t_university_guides.campus_facilities IS '校园设施JSONB';
COMMENT ON COLUMN t_university_guides.dormitory_services IS '水电网与宿舍管理JSONB';
COMMENT ON COLUMN t_university_guides.campus_transportation IS '校园通勤与校外交通JSONB';
COMMENT ON COLUMN t_university_guides.academic_guidance IS '专业与课程核心信息JSONB';
COMMENT ON COLUMN t_university_guides.major_transfer_guidelines IS '转专业原则JSONB';
COMMENT ON COLUMN t_university_guides.major_transfer_constriction IS '转专业限制JSONB';
COMMENT ON COLUMN t_university_guides.academic_support_resources IS '学习支持资源JSONB';
COMMENT ON COLUMN t_university_guides.student_organizations IS '学生组织与社团JSONB';
COMMENT ON COLUMN t_university_guides.campus_events IS '校园活动与竞赛JSONB';
COMMENT ON COLUMN t_university_guides.class_dorm_social IS '班级与宿舍社交JSONB';
COMMENT ON COLUMN t_university_guides.financial_aid IS '奖助勤贷与权益保障JSONB';
COMMENT ON COLUMN t_university_guides.campus_security IS '校园安全与应急处理JSONB';
COMMENT ON COLUMN t_university_guides.health_services IS '医保与心理健康JSONB';
COMMENT ON COLUMN t_university_guides.life_services IS '生活服务JSONB';
COMMENT ON COLUMN t_university_guides.remark IS '备注';
