...
public class PublicWelfarePositionController {
    ...
    @PostMapping("/create")
    @OperationLog(module = "基层服务管理", action = "新增公益性岗位")
    public R<Long> create(@Valid @RequestBody PublicWelfarePositionAddDTO dto) {
        return R.ok(publicWelfarePositionService.add(dto));
    }
    ...
    public R<ImportResultVO> importExcel(@RequestParam("file") MultipartFile file) {
        return R.ok(publicWelfarePositionService.importExcel(file));
    }
}
