...
public class MilitaryPositionController {
    ...
    @PostMapping("/create")
    @OperationLog(module = "体制内招录", action = "新增部队文职岗位")
    public R<Long> create(@Valid @RequestBody MilitaryPositionAddDTO dto) {
        return R.ok(militaryPositionService.add(dto));
    }
    ...
    public R<ImportResultVO> importExcel(@RequestParam("file") MultipartFile file) {
        return R.ok(militaryPositionService.importExcel(file));
    }
}
