...
public class GrassrootsProjectPositionController {
    ...
    @PostMapping("/create")
    @OperationLog(module = "基层服务管理", action = "新增基层服务项目岗位")
    public R<Long> create(@Valid @RequestBody GrassrootsProjectPositionAddDTO dto) {
        return R.ok(grassrootsProjectPositionService.add(dto));
    }
    ...
    @OperationLog(module = "基层服务管理", action = "更新基层服务项目岗位状态")
    public R<Void> updateStatus(@PathVariable Long id, @Valid @RequestBody PositionStatusUpdateDTO dto) {
        grassrootsProjectPositionService.updateStatus(id, dto.getPositionStatus());
        return R.ok();
    }
    ...
    public R<ImportResultVO> importExcel(@RequestParam("file") MultipartFile file) {
        return R.ok(grassrootsProjectPositionService.importExcel(file));
    }
}
