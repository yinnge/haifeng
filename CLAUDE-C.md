...
## 角色
你是一个拥有10年经验的JAVA高级程序员，根据需求进行以下操作

## 用户C端界面：【限制管理员才能看到】
C端模块有：

首页管理[父模块]--公告列表[子模块]--规划师列表[子模块]--培训机构列表[子模块]--界面管理[子模块]
用户管理[父模块]--个人中心[子模块]--收藏列表[子模块]--点赞列表[子模块]--消息通知[子模块]--浏览记录[子模块]
院校管理[父模块]--院校列表[子模块]--校园图册[子模块]--院校适应指南[子模块]--院系详情[子模块]--实验室列表[子模块]--学科评估[子模块]
资源管理[父模块]--资源列表[子模块]
专业管理[父模块]--专业列表[子模块]--专业详情[子模块]--考研专业[子模块]
城市管理[父模块]--城市列表[子模块]
行业管理[父模块]--行业列表[子模块]
专业组管理[父模块]--专业组录取列表[子模块]--专业组明细列表[子模块]--选科要求列表[子模块]
算法配置管理[父模块]--建省份改革配置[子模块]--院校标签字典[子模块]--一分一段位次[子模块]--批次分数线[子模块]
算法约束模块[父模块]--约束字典[子模块]--专业约束关联[子模块]--安全系数[子模块]
特殊通道模块[父模块]--特殊招生通道列表[子模块]--通道详情[子模块]--通道-大学[子模块]--强基计划列表[子模块]
竞赛证书管理[父模块]--科研竞赛列表[子模块]--职业技能证书列表[子模块]
企业管理[父模块]--企业列表[子模块]--企业行业关联[子模块]
公务员考试管理[父模块]--公务员考试列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
事业编管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
部队文职管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
选调生管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
教师招聘管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
医疗卫生招聘管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
银行/金融系统招聘管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
基层服务管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
社区工作管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
公益性岗位管理[父模块]--岗位列表[子模块]--公告列表[子模块]--备考指南列表[子模块]
...
### 列表规范
用到page展示列表，提供列表页的分页参数：10，20，30，50，100
