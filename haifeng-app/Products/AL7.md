...
- 限制仅在**创建新方案时**检查（`getOrCreatePlan` 方法），不会阻止删除、修改等操作
- 当 `planId` 为空时，系统自动获取最近方案或创建新方案，此时触发限制检查
- 达到上限时返回错误码 `400`，消息：`"当前会员类型最多允许 X 个志愿表"`
- **删除方案后名额释放**：删除志愿表后，该用户的已用名额减 1，可创建新方案
- 限制基于 `memberType`（来自 `SecurityUtil.getCurrentMemberType()`），即 JWT 中的会员类型

**方案自动命名规则：**

- 新建方案名称默认为 `"我的志愿方案{N}"`，N = 当前已有方案数 + 1
- 例：已有 2 个方案，新建时自动命名为 `"我的志愿方案3"`

**限制检查时序图：**

```
用户调用 add-majors (planId=null)
    │
    ├─ 1. 查询该用户最近方案 (SELECT ... FOR UPDATE)
    │      ├─ 存在 → 直接使用该方案（不检查限制）
    │      └─ 不存在 → 进入创建流程
    │
    └─ 2. 创建新方案前检查限制
           ├─ count < maxPlans → 创建新方案
           └─ count >= maxPlans → 抛出异常 400
```
...
- 事务保护，任一步骤失败整体回滚
- **名额释放**：删除方案后，该用户的已用方案名额减 1，可创建新方案（限制仅在创建时检查）
...
- 导出状态在 `export` 和 `save` 接口调用后需刷新

### 21.9 志愿表数量限制处理

**前端需关注的场景：**

1. **创建方案时**：首次添加专业（不传 `planId`）时，后端自动创建方案。若已达上限，返回 `400` 错误，前端应提示用户升级会员或删除已有方案。

2. **删除方案后**：方案被删除后名额立即释放，前端可刷新 `my-plans` 列表后引导用户创建新方案。

3. **方案列表展示**：`my-plans` 返回所有未删除方案，前端可根据列表长度和会员类型判断是否已满。
...
用户点击"新建方案"
    │
    ├─ 调用 GET /my-plans 获取当前方案数
    │
    ├─ 对比会员类型限制（normal=1, pro=5, vip=10）
    │      ├─ 未满 → 允许新建，调用 add-majors (planId=null)
    │      └─ 已满 → 提示"已达方案上限，请删除旧方案或升级会员"
    │
    └─ 新建成功后刷新方案列表
```
...
| 错误码 | 场景 | 建议提示 |
|--------|------|---------|
| 400 | 方案数超限 | "当前会员类型最多允许 X 个志愿表，请删除旧方案后重试" |
| 400 | 档位专业数超限 | "X 档专业已选 N 个，最多 M 个" |
| 400 | 重复添加专业 | "以下专业已在志愿表中" |
| 400 | 禁级别专业 | "该专业存在硬性报考限制，不允许添加" |进度
