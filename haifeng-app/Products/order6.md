...
> 本模块**无任何模糊查询**（LIKE / 部分匹配），所有筛选参数（`tag`、`region`）以及路径参数 `id` 均为**精准查询**（`=` 精确匹配）。详见文末「模糊查询 vs 精准查询字段总览」。
...
**功能描述**：分页查询培训机构（`t_institutions`），按 `sort_order ASC, id DESC` 排序。本接口**无业务筛选字段**（无 `tag` / `region` 之类的概念），仅支持分页。公开访问，30 分钟 Redis 缓存。
...
> `InstitutionQueryDTO` 虽无业务字段但仍单独建立（`extends BasePageQueryDTO`），保持三模块对称；未来扩展筛选条件时无需改 Controller 签名。
...
- **本模块无 `LIKE` 查询**：所有筛选条件都是 `=` 精确匹配；前端如果想做"标签搜索下拉联想"等模糊场景，本期不提供。
