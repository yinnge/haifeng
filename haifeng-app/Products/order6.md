...
| 站点信息（site） | 获取站点基本信息（ICP、联系方式、社交链接） | 公开访问 |
...
| size | Integer | 否 | 10 | 每页条数，10–100（`@Min(10) @Max(100)`，推荐档位：10/20/30/50/100） |
...
|------|------|----------|
| 200 | 成功 | 正常返回 |
| 400 | 参数错误 | `page < 1` / `size < 10` / `size > 100` / `region` 不在 `ProvinceEnum` 中文 desc 中 |
...
| size | Integer | 否 | 10 | — | 每页条数（`@Min(10) @Max(100)`） |
...
**功能描述**：获取站点基本信息（ICP备案号、联系方式、社交账号链接），公开访问，1小时 Redis 缓存。
...
| 权限 | 公开 |
| 缓存 | Redis 缓存 1 小时 |
| Service | `SiteServiceImpl.getSiteInfo()` |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| siteIcp | String | 网站 ICP 备案号 |
| contactUrl | ContactUrl | 联系链接对象 |
| contactUrl.wechat | String | 微信二维码 URL |
| contactUrl.weibo | String | 微博主页 URL |
| contactUrl.zhihu | String | 知乎主页 URL |
| contactUrl.douyin | String | 抖音主页 URL |
| contactUrl.bilibili | String | B站主页 URL |
| basicMessage | BasicMessage | 基本联系信息对象 |
| basicMessage.address | String | 公司地址 |
| basicMessage.phone | String | 联系电话 |
| basicMessage.email | String | 联系邮箱 |
| basicMessage.consultationTime | String | 咨询时间 |
...
```json
{
  ...
  "msg": "success",
  "data": {
    "siteIcp": "京ICP备2024000001号",
    "contactUrl": {
      "wechat": "https://cdn.haifeng.com/wechat-qr.png",
      "weibo": "https://weibo.com/haifeng",
      "zhihu": "https://zhihu.com/haifeng",
      "douyin": "https://v.douyin.com/haifeng",
      "bilibili": "https://space.bilibili.com/haifeng"
    },
    "basicMessage": {
      "address": "北京市海淀区中关村大街 1 号",
      "phone": "400-888-8888",
      "email": "service@haifeng.com",
...
> **本模块仅培训机构 `name` 使用模糊查询（`LIKE` / 部分匹配），其余字段均为精准查询。**
...
GET  /api/v1/app/home/site-info                  [公开]   站点信息
```
...
haifeng-app/vo/home/
  ...
  └── SiteInfoVO.java                                   { siteIcp, contactUrl, basicMessage }
```
...
**Q：列表接口是否有最大 size 限制？**
A：有。`size` 取值范围 10–100（`@Min(10) @Max(100)`），超出会被 `BasePageQueryDTO` 校验拦截返回 400。
