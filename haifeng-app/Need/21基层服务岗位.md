...
### 任务1【完成对基层服务项目岗位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_grassroots_project_position表的：id,project_type,year,position_name,service_type,organizing_dept,service_unit,province,city,county,township,service_period,education_requirement,major_requirement,age_limit,recruitment_count,grad_year_requirement,position_status,political_status

支持模糊查询的字段：position_name,organizing_dept,service_unit
支持精准查询的字段：project_type,year,service_type,province,city,county,township,education_requirement,major_requirement,grad_year_requirement,political_status,position_status
...
### 任务2【完成对社区工作者岗位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_community_position表的：id,community_name,district,position_name,position_type,province,city,education_requirement,age_limit,recruitment_count,major_requirement,work_experience,position_status

支持模糊查询的字段：position_name,street_office,community_name,supervising_dept,district,work_location,major_requirement
支持精准查询的字段position_type,employment_type,province,city,education_requirement,political_status,work_experience,position_status
...
### 任务3【完成公益性岗位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_public_welfare_position表的developing_unit，employing_unit，position_name，position_category，province，city，district，education_requirement，recruitment_count，monthly_salary，household_requirement，contract_period

支持模糊查询的字段：position_name，developing_unit，employing_unit，province，city
支持精准查询的字段：position_category，province，city，district，education_requirement，position_status，target_group
