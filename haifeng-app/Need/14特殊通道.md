...
### 任务1【完成对特殊招生通道内容表的查询】
接口1：前端分页展示内容（不需要登录）(只展示is_active=true的数据)
实现分页查询t_special_channel表的：id,channel_code,channel_name,subtitle,filter_label,display_type
支持精准查询字段：display_type,channel_name

接口2：详情（需要登录）(只展示is_active=true的数据)
根据分页查询返回的的id查询t_special_channel表的：id,channel_code,channel_name,subtitle,filter_label,display_type,content



### 任务2【完成对通道-大学关联表的查询】
接口1：前端分页展示内容（不需要登录）(只展示is_active=true的数据)
实现根据一个channel_code分页查询t_special_channel_university表的：university_id，university_name，year，region_tag，signup_start，signup_end
支持模糊查询的字段:channel_name
支持精准查询的字段:region_tag
支持范围查询的字段：signup_start,signup_end
(范围查询具体逻辑:
传入 signupStart → signup_start >= ?（报名开始时间晚于某天）
传入 signupEnd → signup_end <= ?（报名截止时间早于某天）
)
注意点：region_tag要用枚举类：province【已实现】,在"D:\0code\haifeng\backend\haifeng\haifeng-common\src\main\java\com\haifeng\common\enums\ProvinceEnum.java"

接口2：展示通道-大学关联表的详情数据(只展示is_active=true的数据)（需要登录）
根据分页查询返回的的id，查询t_special_channel_university表的：id,channel_code,channel_name,university_id,university_name,year,region_tag,signup_start,signup_end,official_url,brochure_title,brochure_content




### 任务3【完成强基计划入围/录取数据表的查询】
接口1：前端分页展示内容（不需要登录）(只展示is_active=true的数据)
实现分页查询t_strong_base_score表的id,university_id,university_name,year,province,subject_type,major_name,major_code,entry_score,entry_score_type,entry_ratio,admission_score,plan_count,admission_count
支持模糊查询字段:university_name,major_name,major_code
支持精准查询字段：year,province,subject_type,entry_score_type

接口2：详情（需要登录）(只展示is_active=true的数据)
根据分页查询的id查询t_strong_base_score表的id,university_id,university_name,year,province,subject_type,major_name,major_code,entry_score,entry_score_type,entry_formula,entry_ratio,admission_score,admission_formula,plan_count,admission_count,remark

任务3的province要用到枚举类已实现，在"D:\0code\haifeng\backend\haifeng\haifeng-common\src\main\java\com\haifeng\common\enums\ProvinceEnum.java"

### 任务4【完成强基计划院校配置表的查询】
接口1：详情（需要登录）
根据根据university_id查询的查询t_strong_base_university表的id,university_id,university_name,is_pilot,pilot_year,official_url,signup_url,test_before_score,default_entry_ratio,default_admission_formula,available_majors,special_notes
