...
### 任务6【完成分页展示校园图册表的信息】（新增）（权限：需要vip）
数据库表在D:\0code\haifeng\backend\haifeng\haifeng-admin\src\main\resources\db\migration\V11__create_admission_group.sql
接口1：根据university_id分页查询t_admission_group表，查出近5年的数据（year是desc取5年的），
支持省份（用枚举），batch精准查询，city_name模糊查询，查询所有

接口2：再in查询t_admission_major_score表，根据group_id去查，返回所有字段 
复用之前的citycontroller和专业的controller的根据id去查询详情
### 注意事项
