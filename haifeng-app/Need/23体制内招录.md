...
### 任务1【完成对公务员考试职位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_civil_position表的：position_name,exam_type,recruiting_dept,major_requirement,min_education,degree_requirement,political_status,exam_category,work_location,reg_start_date,reg_end_date,reg_status,applicant_count

支持模糊查询的字段：position_name,recruiting_dept,major_requirement,work_location
支持精准查询的字段：exam_type,recruiting_dept,position_code,dept_code,major_requirement,min_education,degree_requirement,political_status,exam_category,work_location
...
### 任务2【完成对事业编职位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_institution_position表的：position_name,supervising_dept,institution,work_location,province,exam_category,position_type,education_requirement,degree_requirement,age_limit,recruitment_count,salary_range,reg_deadline,special_position,position_status

支持模糊查询的字段：position_name,supervising_dept,institution,work_location,province
...
### 任务3【完成部队文职岗位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_military_position表的position_name,employer_unit,department,position_type,work_location,salary_range,major_requirement,education_requirement,reg_deadline,position_status

支持模糊查询的字段：position_name,employer_unit,department,major_requirement
支持精准查询的字段：position_type,education_requirement,position_status,work_location
...
### 任务4【完成选调生岗位表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_selection_position表的position_name,selection_type,year,province,organizing_dept,target_unit,work_location,training_direction,education_requirement,degree_requirement,major_requirement,university_requirement,political_status,age_limit,recruitment_count,reg_start_date,reg_end_date,position_status

支持模糊查询的字段：position_name,target_unit,work_location,major_requirement,
支持精准查询的字段：selection_type,year,province,education_requirement,major_requirement,degree_requirement,political_status,age_limit,position_status
