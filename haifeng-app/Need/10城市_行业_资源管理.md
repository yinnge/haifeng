...
### 任务1【完成对城市主表的查询】
接口1：分页展示城市主表的:id（id不展示）,city_name,province,region,city_intro,college_count,key_college_count,resident_population,gdp,is_deleted。
...
### 任务2【完成对行业主表的查询】
接口1:分页展示行业主表的id（id不展示）,industry_name,category,description,annual_growth_rate,market_scale,talent_gap,investment_heat,is_deleted。
