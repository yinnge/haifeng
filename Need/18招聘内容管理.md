...
### 需创建枚举
在D:\exeProject\ideaProjects\Project-HaiFeng\haifeng-common\src\main\java\com\haifeng\common\enums下创建枚举类
#### noticeCategory（公告类别）
...
### 任务2【完成对统一公告表的查询】
接口1：前端分页展示内容（不需要登录）
实现分页查询t_notice表的：id,title,summary,publish_date,view_count,notice_category,province,city,year,reg_start_date,reg_end_date,recruitment_count
