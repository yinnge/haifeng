...
**删除行为：** 软删除，设置 `is_deleted = TRUE`，记录仍保留在数据库中。恢复方式：通过新增同省份配置自动恢复已删除记录。
...
| 类型 | 必填 | 校验规则 | 说明 |
|------|:----:|----------|------|
| Array\<String\> | ✓ | @NotEmpty, @Size(max=100) | ID数组，单次最多100条 |

**删除行为：** 批量软删除，单条 SQL 执行，性能优于逐条更新。恢复方式：同上。
...
**删除行为：** 软删除，设置 `is_deleted = TRUE`，记录仍保留在数据库中。恢复方式：通过新增同业务键记录自动恢复已删除记录。
...
| 科类 | subjectType | String | 是 | subject_type VARCHAR(20) NOT NULL | 枚举：理科/物理类/文科/历史类/不分文理 |
| 分数 | score | Short | 是 | score SMALLINT NOT NULL | 整数 |
| 位次 | rank | Integer | 是 | rank INTEGER NOT NULL | 非负整数 |
| 同分人数 | sameScoreCount | Integer | 否 | same_score_count INTEGER | 非负整数 |
| 累计人数 | cumulativeCount | Integer | 否 | cumulative_count INTEGER | 非负整数 |
...
| 科类 | 理科、物理类、文科、历史类、不分文理 | 代码枚举校验（5个合法值） | 必填，为空报错 |
...
|------|--------|------|------|
| 分数 | SMALLINT | - | 必填 |
| 位次 | INTEGER | ≥ 0 | 必填，不能为负数 |
| 同分人数 | INTEGER | ≥ 0 | 可空，为空存null |
| 累计人数 | INTEGER | ≥ 0 | 可空，为空存null |

**唯一性约束（DB层）**

| 约束名 | 表 | 字段组合 | 说明 |
|--------|-----|---------|------|
| uk_score_rank | t_score_rank | province + year + subject_type + score | 同省同年同科类同分数唯一 |

#### 校验规则汇总

| 校验类型 | 规则 | 校验层级 | 说明 |
|----------|------|----------|------|
| 必填 | 省份、年份、科类、分数、位次 | 代码 | 为空则记录错误 |
| 年份范围 | 2000-2100 | 代码 | 超出范围则记录错误 |
| 省份枚举 | 31个标准省市名 | 代码 | 精确匹配，不合法则报错 |
| 科类枚举 | 理科/物理类/文科/历史类/不分文理 | 代码 | 精确匹配，不合法则报错 |
| 数值非负 | 位次、同分人数、累计人数 | 代码 | 均不能为负数 |
| Excel内去重 | 省份+年份+科类+分数（业务键） | 代码 | Excel内重复则报错 |
| DB已存在检查 | 省份+年份+科类+分数 | 代码 | 已存在（未删除）的记录拒绝导入 |
| 软删除记录恢复 | 省份+年份+科类+分数 | 代码 | 软删除记录自动恢复并更新，不报错 |
...
2. **软删除记录恢复机制**：
   - 若数据库中存在已软删除的相同业务键记录（province + year + subject_type + score），不会报错
   - 恢复该记录：设置 `is_deleted = false`，并更新 `rank`、`sameScoreCount`、`cumulativeCount` 字段
   - 恢复的记录计入返回的处理条数
...
| 年份 | year | Short | 是 | year SMALLINT NOT NULL | 整数，范围 2000-2100 |
| 科类 | subjectType | String | 是 | subject_type VARCHAR(20) NOT NULL | 枚举：理科/物理类/文科/历史类/不分文理 |
| 批次 | batch | String | 是 | batch VARCHAR(50) NOT NULL | 自由文本（本科批/专科批/本科一批/本科二批等） |
| 分数线 | scoreLine | Integer | 是 | score_line INTEGER NOT NULL | 整数，范围 0-900 |
| 位次线 | rankLine | Integer | 否 | rank_line INTEGER | 整数，范围 0-9999999，可空 |
| 备注 | remark | String | 否 | remark VARCHAR(200) | 自由文本，最长200字符 |
...
|------|------|------|------|--------|--------|------|
| 广东 | 2025 | 物理类 | 本科批 | 445 | 200000 | 2025年广东物理类本科批省控线 |
| 广东 | 2025 | 物理类 | 专科批 | 180 | 380000 | |
| 广东 | 2025 | 历史类 | 本科批 | 430 | 85000 | |
| 广东 | 2025 | 历史类 | 专科批 | 160 | 350000 | |
| 北京 | 2025 | 不分文理 | 本科批 | 425 | 38000 | |
...
| 字段 | DB类型 | 范围 | 说明 |
|------|--------|------|------|
| 分数线 | INTEGER | 0-900 | 必填，超出范围则报错 |
| 位次线 | INTEGER | 0-9999999 | 可空，为空存null；填写时超出范围则报错 |

**唯一性约束（DB层）**

| 约束名 | 表 | 字段组合 | 说明 |
|--------|-----|---------|------|
| uk_batch_score_line | t_batch_score_line | province + year + subject_type + batch | 同省同年同科类同批次唯一 |

#### 校验规则汇总

| 校验类型 | 规则 | 校验层级 | 说明 |
|----------|------|----------|------|
| 必填 | 省份、年份、科类、批次、分数线 | 代码 | 为空则记录错误 |
| 字段长度 | province≤20, subjectType≤20, batch≤50, remark≤200 | 代码 | 超长则记录错误 |
| 年份范围 | 2000-2100 | 代码 | 超出范围则记录错误 |
| 省份枚举 | 31个标准省市名 | 代码 | 精确匹配，不合法则报错 |
| 科类枚举 | 理科/物理类/文科/历史类/不分文理 | 代码 | 精确匹配，不合法则报错 |
| 分数线范围 | 0-900 | 代码 | 超出范围则记录错误 |
| 位次线范围 | 0-9999999（可选字段，为空不校验） | 代码 | 非空且超出范围则记录错误 |
| Excel内去重 | 省份+年份+科类+批次（业务键） | 代码 | Excel内重复则报错 |
| DB已存在检查 | 省份+年份+科类+批次 | 代码 | 已存在（未删除）的记录拒绝导入 |
| 软删除记录恢复 | 省份+年份+科类+批次 | 代码 | 软删除记录自动恢复并更新，不报错 |
| 行数上限 | 单次导入不超过1000条 | 代码 | 超限则直接拒绝 |
| 错误截断 | 错误超过20条时仅展示前20条 + 总数 | 代码 | 格式：`前20条错误; ... 等N条错误` |
| 文件类型 | .xlsx 或 .xls | 代码 | 其他格式拒绝 |
...
1. **两阶段处理**：先校验所有数据，校验通过后批量插入（要么全部成功，要么全部失败）
2. **软删除记录恢复机制**：
   - 若数据库中存在已软删除的相同业务键记录（province + year + subject_type + batch），不会报错
   - 恢复该记录：设置 `is_deleted = false`，并更新 `scoreLine`、`rankLine`、`remark` 字段
3. **已存在记录拒绝**：若数据库中存在**未删除**的相同业务键记录，拒绝导入并报错
4. **事务保证**：方法标注 `@Transactional(rollbackFor = Exception.class)`，任何异常全部回滚
5. **行数上限**：单次导入不超过1000条记录
...
| 字段 | 类型 | 说明 |
|------|------|------|
| data | null | 固定返回 null（接口不返回成功条数，语义为"要么全成功要么全失败"） |
...
{
  "code": 400,
  "msg": "Excel文件读取失败，请检查文件格式与单元格数据类型",
  "data": null,
  "timestamp": 1715241600000
}
```
...
2. **软删除记录会被自动恢复**：若数据库中存在已软删除的相同业务键记录，导入时会自动恢复并更新字段值，不会报错
3. **已存在（未删除）的记录会拒绝导入**：若数据库中存在未删除的相同业务键记录，该行会报错
4. **批次为自由文本**：不校验批次枚举值，可填写"本科批"、"专科批"、"本科一批"、"本科二批"等
5. **接口不返回成功条数**：`data` 字段固定返回 null，语义为"要么全成功要么全失败"；成功/恢复条数仅记录在服务端日志中
...
|--------|------|
| 200 | 成功 |
| 400 | 参数错误 / 数据校验失败 / 数据已存在 / 数据已被其他人修改（乐观锁冲突） |
| 404 | 记录不存在 |
| 401 | 未登录 / Token过期 |
| 403 | 无权限 |
| 500 | 服务器内部错误 |
