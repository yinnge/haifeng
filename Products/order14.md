...
| 操作 | HTTP方法 | 路径 | 说明 |
|------|----------|------|------|
| 硬删除 | DELETE | `/{id}` | 物理删除记录，数据不可恢复。删除通道前会校验是否有关联的大学记录 |
| 批量硬删除 | POST | `/batch-delete` | 批量物理删除（最多100条），请求体 `{"ids": [...]}` |
| 禁用/启用 | PUT | `/{id}/toggle` | 切换 isActive 状态 |

> **风险提示**：当前 `t_special_channel` 和 `t_special_channel_university` 均无 `is_deleted` 字段，删除为物理删除。若其他表（如收藏、浏览记录等）引用了这些记录的 `channel_code` 或 `id`，硬删除会导致孤儿引用。建议后续添加 `is_deleted` 字段改为软删除。
...
| DELETE | `/{id}` | 删除通道（硬删除，未评估外键引用风险） |
| POST | `/batch-delete` | 批量删除（硬删除，最多100条，未评估外键引用风险） |
...
**业务说明**：物理删除记录，数据不可恢复。删除前会校验 `t_special_channel_university` 是否有关联记录，如有则拒绝删除。

> **注意**：若其他表引用了该通道的 `channel_code`，硬删除会导致孤儿引用。当前未评估所有外键引用风险。

### 1.7 批量删除

**请求**
```
POST /api/v1/admin/special/channel/batch-delete
Content-Type: application/json
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| ids | Array\<Long\> | 是 | ID数组，不能为空，最多100条 |

**参数限制**：最多 100 条

**业务说明**：批量物理删除。删除前会校验所有通道是否有关联的大学记录，如有则拒绝整批删除。

> **注意**：若其他表引用了这些通道的 `channel_code`，硬删除会导致孤儿引用。
...
> **注意**：当前未评估 `t_special_channel_university` 被其他表引用的风险，硬删除可能导致孤儿引用。

### 2.7 批量删除

**请求**
```
POST /api/v1/admin/special/channel-univ/batch-delete
Content-Type: application/json
...
| PUT | `/{id}/toggle` | 切换启用状态 |
| DELETE | `/{id}` | 删除数据（硬删除） |
| POST | `/batch-delete` | 批量删除（硬删除，最多100条） |
...
> **注意**：universityName 为模糊查询，其他查询条件为精确匹配
...
**请求**
```
POST /api/v1/admin/special/strong-base-score/batch-delete
Content-Type: application/json
...
**请求**
```
POST /api/v1/admin/special/strong-base-univ/batch-delete
Content-Type: application/json
...
| 通道-大学关联 | 无 | 无查询条件 |
| 强基计划数据 | universityName | **模糊查询** |
| 强基计划数据 | year | **精确查询** |
| 强基计划数据 | province | **精确查询** |
| 强基计划数据 | subjectType | **精确查询** |
| 强基院校配置 | universityName | **模糊查询** |
| 强基院校配置 | isPilot | **精确查询** |
