...
> - 企业性质**必填**，必须是：央企、国企、民企、外企、合资
> - Sheet0和Sheet1各自不能超过500行
> - 字段长度限制：企业名称≤200、城市名称≤50、企业类型≤50、Logo地址≤500、官网≤500、总部地区≤100、企业规模≤50、主营业务≤500、招聘状态≤20
> - 招聘类型可选：校招、社招、实习（可为空）
> - 学历要求可选：不限、大专、本科、硕士、博士（可为空）
> - 岗位状态可选：招聘中、已结束（可为空）
> - 岗位标签支持中英文逗号分隔，如：五险一金,弹性工作
> - 薪资单位为k/月，如20表示20k；最低薪资不能大于最高薪资
> - 截止日期格式：yyyy-MM-dd 或 yyyy-MM-dd HH:mm:ss

**Sheet1 注意事项**：
> - 岗位名称必填，最大200字符
> - 字段长度限制：岗位名称≤200、省份≤30、城市≤50、工作地点≤200、专业要求≤500、工作经验≤50、申请链接≤500
> - 岗位状态（如果非空）必须是：招聘中、已结束
> - 最低薪资不能大于最高薪资
...
> **注意事项**：
> - 企业名称必填（≤200字符），必须在 t_enterprise 表中存在
> - 行业名称必填（≤100字符），必须在 t_industry 表中存在
> - 单次导入数量不能超过500行
> - 同一企业不能重复关联同一行业
...
{
  "code": 400,
  "msg": "导入失败：Sheet0第2行：企业名称不能为空；Sheet0第3行：企业性质不能为空；Sheet0第5行：企业名称'腾讯科技'在文件中重复；Sheet0第8行：企业名称'华为技术'已存在于数据库；Sheet0第10行：企业性质'事业单位'不合法，必须是：央企、国企、民企、外企、合资；Sheet0第12行：城市名称长度不能超过50个字符；Sheet1第3行：企业名称'未知企业'在企业数据Sheet中不存在；Sheet1第4行：岗位名称不能为空；Sheet1第6行：岗位状态'暂停'不合法，必须是：招聘中、已结束；Sheet1第8行：最低薪资不能大于最高薪资",
  "data": null,
  "timestamp": 1715500800000
}
```
...
2. 单次导入每个Sheet不能超过500行
3. 企业名称必填且唯一（文件内+数据库），最大200字符
4. 企业性质必填，必须是：央企、国企、民企、外企、合资
5. Sheet0字段长度限制：城市名称≤50、企业类型≤50、Logo地址≤500、官网≤500、总部地区≤100、企业规模≤50、主营业务≤500、招聘状态≤20
6. Sheet1中的企业名称必须在Sheet0中存在
7. 岗位名称必填，最大200字符
8. Sheet1字段长度限制：省份≤30、城市≤50、工作地点≤200、专业要求≤500、工作经验≤50、申请链接≤500
9. 招聘类型（如果非空）必须是：校招、社招、实习
10. 学历要求（如果非空）必须是：不限、大专、本科、硕士、博士
11. 岗位状态（如果非空）必须是：招聘中、已结束
12. 最低薪资不能大于最高薪资
13. 岗位标签支持中英文逗号分隔
14. 任何错误都会导致整个导入回滚
...
{
  "code": 400,
  "msg": "导入失败：第2行：企业名称不能为空；第5行：企业名称长度不能超过200个字符；第8行：行业名称'未知行业'不存在；第10行：企业'腾讯科技'-行业'信息技术'关联在文件中重复；第12行：企业'腾讯科技'-行业'信息技术'关联已存在",
  "data": null,
  "timestamp": 1715500800000
}
```
...
**导入规则**：
1. 单次导入不能超过500行
2. 企业名称必填（≤200字符），必须在 t_enterprise 表中存在（is_deleted=false）
3. 行业名称必填（≤100字符），必须在 t_industry 表中存在（is_deleted=false）
4. 同一企业不能重复关联同一行业（文件内+数据库）
5. 任何错误都会导致整个导入回滚
